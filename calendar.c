...
void display(int nyr,int nmonth,int tdays,int days[])
{
   ...
   tdays++; //incrementing the tdays by 1
    ...
    {
        if(pos==91)
            SetColor(3); //Changing color to dark grey for sunday
        else
    ...
    for(i=22;i<102;i++)
    {
        gotoxy(i,4);printf("%c",200);
        gotoxy(i,17);printf("%c",200);
    }
    ...
    for(i=5;i<17;i++)
    {
        gotoxy(21,i);printf("%c",180);
        gotoxy(102,i);printf("%c",180);
    }
